...
func (m *mockBroadcaster) BroadcastDiffCard(cardID, file, diffContent string, semanticGroups []SemanticGroupInfo, isBinary, isDeleted bool, stats *DiffStats, createdAt int64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// TestCardStreamer_ProcessChunksRaw_SemanticGroups verifies that the live path
// populates semantic groups on broadcast cards.
func TestCardStreamer_ProcessChunksRaw_SemanticGroups(t *testing.T) {
	store, err := storage.NewSQLiteStore(":memory:")
	if err != nil {
		t.Fatalf("failed to create store: %v", err)
	}
...
// TestCardStreamer_StreamPendingCards_SemanticGroups verifies that the replay
// path populates semantic groups.
func TestCardStreamer_StreamPendingCards_SemanticGroups(t *testing.T) {
	store, err := storage.NewSQLiteStore(":memory:")
	if err != nil {
		t.Fatalf("failed to create store: %v", err)
	}
...
// binary cards get semantic groups with binary kind.
...
// TestEnrichFileWithSemantics_NilSafe verifies the enricher handles
// empty/nil input without panicking.
func TestEnrichFileWithSemantics_NilSafe(t *testing.T) {
	// This just verifies the function doesn't panic on empty input
	groups := EnrichFileWithSemantics("card-1", "test.go", "", false, false)
	if len(groups) != 0 {
