...
func TestMetrics_PromotionEvidence(t *testing.T) {
	store := newTestMetricsStore(t)

	err := store.RecordPromotionEvidence(5, "beta", true, `{"crash_free":99.5}`)
	if err != nil {
		t.Fatalf("RecordPromotionEvidence: %v", err)
	}

	// Verify by querying directly
	var count int
	if err := store.db.QueryRow("SELECT COUNT(*) FROM promotion_evidence WHERE phase = 5 AND passed = 1").Scan(&count); err != nil {
